...
async function register() {
  clearMessage()
  try {
    await axios.post('/api/user/users', registerForm)
    loginForm.mobile = registerForm.mobile
    loginForm.password = registerForm.password
    setMessage('註冊成功，請使用手機號碼登入', 'success')
  } catch (error) {
    console.log(error)
    setMessage(getErrorMessage(error, '註冊失敗11'), 'danger')
  }
}
  ...
  <div class="bg-light min-vh-100">
    <nav class="navbar navbar-expand-lg bg-white border-bottom sticky-top">
      <div class="container">
        ...
        <div class="d-flex gap-2 flex-wrap">
          ...
          <button
            class="btn"
            :class="currentPage === 'posts' ? 'btn-primary' : 'btn-outline-primary'"
            @click="showPage('posts')"
          >
            瀏覽發文
          </button>
          <button
            class="btn"
            :class="currentPage === 'create' ? 'btn-primary' : 'btn-outline-primary'"
            @click="showPage('create')"
          >
            新增發文
          </button>
        </div>
    ...
    <main class="container py-4">
      <div class="card mb-3">
        <div class="card-body d-flex justify-content-between align-items-center flex-wrap gap-2">
          <div>
            ...
            <div class="text-muted small">{{ isLoggedIn ? user.mobile : '登入後可以發文與留言' }}</div>
          </div>
      ...
      <section v-if="currentPage === 'posts'">
        ...
        <div v-if="!loading && posts.length === 0" class="alert alert-secondary">
          目前沒有任何發文。
        </div>
        ...
        <article v-for="post in posts" :key="post.postId" class="card mb-3">
          ...
          <div v-if="visibleComments[post.postId]" class="card-footer bg-body-tertiary">
            <h4 class="h6">留言</h4>
            <div v-if="!commentsByPostId[post.postId]?.length" class="text-muted mb-3">
              目前沒有留言。
            </div>
      ...
      <section v-if="currentPage === 'create'" class="card">
        ...
        <form class="card-body" @submit.prevent="createPost">
          ...
          <div class="mb-3">
            ...
            <input v-model="postForm.image" class="form-control" :disabled="!isLoggedIn" type="url" placeholder="https://..." />
          </div>
